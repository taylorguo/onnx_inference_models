...
def validate(val_loader, val_loader_len, model, criterion):
    ...
    for i, (input, target) in enumerate(val_loader):
        ...
        file_name = "D:\\GitHub\\work_2022\\resnet_101\\imagenet_classes.txt"
...
if __name__ == '__main__':
    ...
    print("=> creating model '{}'".format(args.arch))
    model = models.__dict__[args.arch](width_mult=args.width_mult)
    print("=> ONNX converter:  loading pretrained weight '{}'".format(args.weight))
    source_state = torch.load("pretrained/mobilenetv2_1.0-0c6065bc.pth")
    model.load_state_dict(source_state, strict=False)
    torch.onnx.export(model, torch.autograd.Variable(torch.randn(1, 3, 224, 224)), 
                      "MobileNetV2_1.0_224x224_pth.onnx", verbose=False,opset_version=9,
