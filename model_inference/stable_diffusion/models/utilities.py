...
def device_view(t, dtype=None):
    # return cuda.DeviceView(ptr=t.data_ptr(), shape=t.shape, dtype=torch_to_numpy_dtype_dict[t.dtype])
    if dtype == None: dtype = t.dtype
    return numpy.asarray(t, dtype=torch_to_numpy_dtype_dict[dtype])
...
def save_image(images, image_path_dir, image_name_prefix):
    ...
    # images = np.clip(images, 0, 255).transpose(0, 2, 3, 1).dtype(np.uint8)
    print(images.shape, images)
    image = images
    image = np.clip(image / 2 + 0.5, 0, 1)
    image = image.transpose((0, 2, 3, 1))
    print(image[0].shape, image[0])
    # for i in range(images.shape[0]):
    image_path  = os.path.join(image_path_dir, image_name_prefix+str(1)+'-'+str(random.randint(1000,9999))+'.png')
    print(f"Saving image {1} / {images.shape[0]} to: {image_path}")
    Image.fromarray(images[0]).save(image_path)

def save_image_(images, image_path_dir, image_name_prefix):
    """
    Convert a numpy image or a batch of images to a PIL image.
    """
    from PIL import Image
    ...
    image_path  = os.path.join('result-'+str(1)+'-'+str(random.randint(1000,9999))+'.png')
    print(f"Saving image {1} / {images.shape[0]} to: {image_path}")
    # return pil_images
