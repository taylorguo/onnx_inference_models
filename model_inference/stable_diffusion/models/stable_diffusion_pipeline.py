...
import torch
from .utilities import Engine, device_view, save_image, save_image_
...
class StableDiffusionPipeline:
    ...
    def loadEngines(
        ...
        for model_name, obj in self.models.items():
            onnx_path = self.getOnnxPath(model_name, onnx_dir, opt=False)
            print(model_name, onnx_path) #, ORT.get_available_providers())
            onnx_opt_path = self.getOnnxPath(model_name, onnx_dir)
            ...
            if model_name == "vae":
                ort_session = ORT.InferenceSession(onnx_path, providers=providers)
            self.engine[model_name] = ort_session
    ...
    def runEngine(self, model_name, feed_dict):
        ort_session = self.engine[model_name]
        return ort_session.run(None, feed_dict)
    

    def initialize_latents(self, batch_size, unet_channels, latent_height, latent_width):
        latents_dtype = torch.float32 # text_embeddings.dtype
        latents_shape = (batch_size, unet_channels, latent_height, latent_width)
        latents = torch.randn(latents_shape, dtype=latents_dtype, generator=self.generator) # device=self.device, 
    ...
    def encode_prompt(self, prompt, negative_prompt):
        ...
        text_embeddings = self.runEngine('clip', {"input_ids": text_input_ids_inp})[0]
        print(" *** CLIP inference result: \n", text_embeddings)
        ...
        uncond_embeddings = self.runEngine('clip', {"input_ids": uncond_input_ids_inp})[0]
        print(" *** CLIP uncondtional inference result: \n", uncond_embeddings)
    ...
    def denoise_latent(self, latents, text_embeddings, timesteps=None, step_offset=0, mask=None, masked_image_latents=None):
        ...
        for step_index, timestep in enumerate(timesteps):
            ...
            embeddings_inp =  device_view(text_embeddings, dtype=torch.float16)
